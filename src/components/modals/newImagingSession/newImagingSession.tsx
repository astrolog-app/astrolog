...
export default function NewImagingSession({ onClose }: { onClose: () => void }) {
    const { appState } = useAppState();
    const [value, setValue] = useState<string>(appState.preferences.storage.source_directory);
    const [buttonEnabled, setButtonEnabled] = useState<boolean>(value !== "");

    function changeValue(appState: AppState, setAppState: Dispatch<SetStateAction<AppState>>, value: string, path: string) {
        setValue(value);
    }
    ...
    return (
        ...
        >
            <Form {...form}>
                        ...
                        <FormItem>
                            ...
                            <FormControl>
                                <OptionInput value={value} disabled>
                                    <ChangeButton saveAction={changeValue} path="" variant='secondary' />
                                </OptionInput>
                            </FormControl>
                            <FormDescription>
                ...
                />
                <Button className={styles.submitButton} disabled={!buttonEnabled} onClick={detectImagingSessions} type='submit'>Detect Imaging Sessions</Button>
            </Form>
        </Modal>
    );
}
